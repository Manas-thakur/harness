...
from phi_ai.env import DEFAULT_OLLAMA_API_KEY
from phi_coding.credentials import FileCredentialStore, OAuthCredential
from phi_coding.ollama import OllamaModel, discover_ollama_models, ollama_tags_url
...
def test_load_provider_settings_missing_file_uses_ollama_default(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(
        "phi_coding.provider_config.discover_ollama_models_with_capabilities",
        lambda base_url, **kwargs: [OllamaModel(name="qwen2.5:7b", supports_tools=True)],
    )
    settings = load_provider_settings(PhiPaths(home=tmp_path / ".phi"))
...
def test_save_provider_settings_writes_backup_when_replacing(tmp_path: Path) -> None:
    paths = PhiPaths(home=tmp_path / ".phi")
...
def test_save_and_load_provider_settings_round_trip(tmp_path: Path) -> None:
    ...
    assert path == tmp_path / ".phi" / "providers.json"
    assert loaded.default_provider == "local"
    assert loaded.get_provider("local") == settings.get_provider("local")
    assert loaded.scoped_models == settings.scoped_models
    assert loaded.get_provider("ollama").name == "ollama"
...
def test_load_provider_settings_merges_builtin_model_catalog(tmp_path: Path) -> None:
    ...
    phi_home.mkdir()
    (phi_home / "providers.json").write_text(
        """
{
  "default_provider": "huggingface",
  "providers": [
...
""".strip(),
    ...
    settings = load_provider_settings(PhiPaths(home=phi_home))
...
""".strip(),
    ...
    store = FileCredentialStore(phi_home / "credentials.json")
...
def test_ollama_provider_does_not_require_api_key() -> None:
    ...
    assert isinstance(ollama, OpenAICompatibleProviderConfig)
    assert ollama.requires_api_key is False
    assert ollama.credential_name is None
    assert provider_has_usable_credentials(ollama) is True


def test_ollama_provider_resolves_api_key_without_env(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("OLLAMA_API_KEY", raising=False)
    settings = ProviderSettings()
    ollama = settings.get_provider("ollama")
    config = openai_compatible_config_from_provider(ollama)
    assert config.api_key == DEFAULT_OLLAMA_API_KEY
    assert config.base_url == "http://localhost:11434/v1"


def test_ollama_provider_uses_env_api_key_when_set(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_ollama_tags_url_strips_v1_suffix() -> None:
    assert ollama_tags_url("http://localhost:11434/v1") == "http://localhost:11434/api/tags"
    assert ollama_tags_url("http://localhost:11434/v1/") == "http://localhost:11434/api/tags"
    assert ollama_tags_url("http://host:8080") == "http://host:8080/api/tags"


def test_discover_ollama_models_returns_empty_when_unreachable() -> None:
    assert discover_ollama_models("http://127.0.0.1:1/v1", timeout_seconds=0.5) == []


def test_discover_ollama_models_parses_tags(monkeypatch: pytest.MonkeyPatch) -> None:
    class FakeResponse:
        status_code = 200
    ...
    import phi_coding.ollama as ollama_module

    monkeypatch.setattr(ollama_module, "get", FakeHttpx().get)
    models = discover_ollama_models("http://localhost:11434/v1")
    assert models == ["qwen2.5:7b", "llama3:8b"]

    from phi_coding.ollama import discover_ollama_models_with_capabilities

    detailed = discover_ollama_models_with_capabilities("http://localhost:11434/v1")
    assert [m.name for m in detailed] == ["qwen2.5:7b", "llama3:8b"]
    assert detailed[0].supports_tools is True
    assert detailed[1].supports_tools is False
