...
from phi_coding.tui import TuiEventAdapter, TuiState
from phi_coding.tui.state import format_tool_call_block, format_tool_result_block
...
def test_tui_adapter_builds_user_items_from_streamed_messages() -> None:
    ...
    adapter.apply(MessageEndEvent(message=UserMessage(content="Hello Phi")))

    assert state.assistant_buffer == ""
    assert [(item.role, item.text) for item in state.items] == [("user", "Hello Phi")]
...
def test_tool_call_blocks_use_human_readable_invocations() -> None:
    ...
    assert (
        format_tool_call_block(
            ToolCall(id="call-2", name="edit", arguments={"path": "src/phi_coding/tui/app.py"})
        )
        == "→ edit src/phi_coding/tui/app.py"
    )
    assert (
        format_tool_call_block(
            ToolCall(
