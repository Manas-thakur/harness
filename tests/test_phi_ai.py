...
from phi_agent import AgentTool, AgentToolResult, SimpleCancellationToken, ToolCall, UserMessage
...
async def test_openai_compatible_provider_formats_request_and_streams_text() -> None:
    ...
    assert payload["messages"] == [
        {"role": "system", "content": "You are Phi."},
...
async def test_openai_compatible_provider_retries_without_tools_when_unsupported() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        requests.append(request)
        payload = loads(request.content)
        if call_count == 1 and payload.get("tools"):
            return httpx.Response(
                400,
    ...
    async def executor(arguments, signal=None):
        return AgentToolResult(ok=True, content=str(arguments))
    ...
    assert "tool_support" in types
    tool_support_event = next(e for e in events if e.type == "tool_support")
    assert isinstance(tool_support_event, ProviderToolSupportEvent)
    assert tool_support_event.supported is False

    first_payload = loads(requests[0].content)
    second_payload = loads(requests[1].content)
    assert first_payload.get("tools")
    assert "tools" not in second_payload
    assert [event.type for event in events if event.type in ("text_delta", "response_end")] == [
        "text_delta",
        "text_delta",
        "response_end",
    ]
...
async def test_openai_codex_provider_formats_request_and_streams_text() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        assert payload["stream"] is True
        assert payload["instructions"] == "You are Phi."
    ...
    assert request.headers["originator"] == "phi"
...
async def test_anthropic_provider_formats_request_and_streams_text() -> None:
    ...
    assert payload["stream"] is True
    assert payload["system"] == "You are Phi."
