...
from phi_coding import session as coding_session_module
from phi_coding.session import parse_terminal_command
...
async def test_session_export_defaults_to_cwd(tmp_path: Path) -> None:
    storage = JsonlSessionStorage(tmp_path / ".phi" / "sessions" / "session-1.jsonl")
    session = await CodingSession.load(_config(tmp_path, FakeProvider([]), storage))
...
async def test_prompt_logs_unexpected_agent_call_exception(tmp_path: Path) -> None:
    storage = JsonlSessionStorage(tmp_path / "session.jsonl")
    phi_paths = PhiPaths(home=tmp_path / "phi-home", agents_home=tmp_path / "agents-home")
    session = await CodingSession.load(
        CodingSessionConfig(
            ...
            resource_paths=PhiResourcePaths(root=phi_paths.home, paths=phi_paths),
        )
    )
    ...
    log_path = phi_paths.agent_calls_log_path
...
async def test_prompt_logs_error_event_diagnostic_data(tmp_path: Path) -> None:
    ...
    provider = FakeProvider(
        [
            [
                ProviderErrorEvent(
                    message="provider failed", data={"status_code": 400, "body": "bad request"}
                )
            ]
        ]
    )
...
async def test_session_builds_system_prompt_when_system_is_omitted(tmp_path: Path) -> None:
    ...
    config = CodingSessionConfig(
        ...
        storage=storage,
        cwd=tmp_path,
        resource_paths=PhiResourcePaths(root=resource_root, agents_root=None),
    )
    session = await CodingSession.load(config)
...
async def test_session_touches_session_manager_after_persisting_messages(tmp_path: Path) -> None:
    storage = JsonlSessionStorage(tmp_path / "session.jsonl")
    manager = SessionManager(PhiPaths(home=tmp_path / ".phi", agents_home=tmp_path / ".agents"))
    ...
    config = CodingSessionConfig(
        ...
        resource_paths=PhiResourcePaths(root=tmp_path / "resources", agents_root=None),
    )
    session = await CodingSession.load(config)
...
async def test_session_reload_skips_provider_settings_refresh(
    ...
    def fail_load_provider_settings(paths: PhiPaths | None = None) -> ProviderSettings:
        del paths
...
async def test_session_switch_uses_session_credential_store(
    ...
    phi_paths = PhiPaths(home=tmp_path / "phi-home", agents_home=tmp_path / "agents-home")
    FileCredentialStore(phi_paths.home / "credentials.json").set("openai", "stored-key")
    ...
    assert credential_store_paths == [phi_paths.home / "credentials.json"]
...
async def test_session_toggles_and_cycles_scoped_models(
    ...
    saved = json.loads((phi_paths.home / "providers.json").read_text(encoding="utf-8"))
...
async def test_session_toggle_scoped_model_preserves_newer_provider_file_changes(
    ...
    save_provider_settings(newer_settings, phi_paths)
    session = await CodingSession.load(
        CodingSessionConfig(
            provider=FakeProvider([]),
            model="qwen",
    ...
    saved = coding_session_module.load_provider_settings(phi_paths)
...
async def test_session_set_model_persists_default_provider_model(
    ...
    monkeypatch.setenv("HOME", str(tmp_path))
    phi_paths = PhiPaths(home=tmp_path / ".phi", agents_home=tmp_path / ".agents")
    provider_config = OpenAICompatibleProviderConfig(
        name="openai",
...
async def test_session_set_model_preserves_newer_provider_file_changes(
    ...
    saved = coding_session_module.load_provider_settings(phi_paths)
    assert saved.get_provider("openai").default_model == "gpt-5-mini"
    assert saved.get_provider("openrouter").headers == {"X-Title": "Phi"}
    assert saved.scoped_models == (
