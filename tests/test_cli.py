...
from phi_agent import AssistantMessage, UserMessage
...
def test_version_command() -> None:
    ...
    assert result.stdout.strip() == "phi 0.1.0"
...
def test_providers_command_lists_default_provider(
    ...
    assert result.exit_code == 0
    assert "*\tollama\topenai-compatible\tqwen2.5:7b" in result.stdout
...
def test_render_provider_settings_shows_credential_source(
    ...
    assert "*\tstored\topenai-compatible\tqwen2.5:7b" in output
...
def test_setup_command_warns_when_api_key_env_is_missing(
    ...
    assert result.exit_code == 0
    assert "Set MISSING_API_KEY before running Phi with this provider." in result.stderr
