...
from phi_coding.commands import CommandResult
from phi_coding.credentials import FileCredentialStore, OAuthCredential
from phi_coding.prompt_templates import PromptTemplate
...
from phi_coding.session import ModelChoice, SessionTreeChoice, TerminalCommandResult
from phi_coding.session_manager import CodingSessionRecord
from phi_coding.skills import Skill, format_skill_invocation
from phi_coding.system_prompt import ProjectContextFile
from phi_coding.tools import create_coding_tools
from phi_coding.tui import app as tui_app
from phi_coding.tui.app import (
    CommandOutputScreen,
...
from phi_coding.tui.autocomplete import CompletionItem, CompletionState
...
from phi_coding.tui.state import ChatItem
...
def _visible_footer_bindings(app: PhiTuiApp) -> dict[str, str]:
...
def test_session_sidebar_uses_accented_aligned_headers_without_section_borders() -> None:
    ...
    assert str(header.renderable.style) == f"bold {PHI_DARK_THEME.accent}"
    assert " session" in output
    assert " context" in output
    assert "─" in output
    assert "┌" not in output
...
def test_assistant_chat_items_apply_syntax_highlighting_to_code_fences() -> None:
    ...
    assert "\x1b[94;48;2;22;27;34mdef" in output
    assert "\x1b[94;48;2;22;27;34mreturn" in output
...
def test_tui_state_compacts_expanded_skill_messages() -> None:
    skill = Skill(
        name="review",
        path=Path("/workspace/.phi/skills/review.md"),
        content="# Review\nFull noisy instructions.",
            ...
            AssistantMessage(
                ...
                tool_calls=[
                    ToolCall(
                        id="call-1",
                        name="read",
                        arguments={"path": "/workspace/.phi/skills/review.md"},
                    )
                ],
            ),
            ToolResultMessage(
...
def test_dark_theme_markdown_code_uses_aqua_highlight() -> None:
    console = Console(record=True, width=80)
    console.print(render_chat_item(ChatItem(role="assistant", text="Use `phi` here.")))

    output = console.export_text(styles=True)

    assert "38;2;121;192;255" in output
...
def test_assistant_markdown_titles_use_highlight_color_and_left_alignment() -> None:
    ...
    assert _style_color_escape(PHI_DARK_THEME.markdown_heading) in output
...
def test_dark_theme_markdown_links_use_theme_link_color() -> None:
    ...
    assert "38;2;88;166;255" in output
...
def test_dark_theme_markdown_bullets_use_theme_bullet_color() -> None:
    ...
    assert _style_color_escape(PHI_DARK_THEME.markdown_bullet) in output
...
def test_markdown_tables_use_highlight_color_for_headers() -> None:
    ...
    assert "38;2;227;179;65" in output
...
async def test_textual_markdown_widget_uses_theme_link_style() -> None:
    ...
    assert markdown.phi_link_style == PHI_DARK_THEME.markdown_link


def test_textual_markdown_uses_theme_highlight_and_aqua_inline_code() -> None:
    variables = _theme_css_variables(PHI_LIGHT_THEME)

    assert variables["phi-markdown-highlight"] == PHI_LIGHT_THEME.markdown_heading
    assert variables["phi-markdown-table-header"] == PHI_LIGHT_THEME.markdown_table_header
    assert variables["phi-markdown-table-border"] == PHI_LIGHT_THEME.markdown_table_border
    assert variables["phi-markdown-inline-code"] == PHI_LIGHT_THEME.markdown_inline_code
    assert (
        variables["phi-markdown-code-block-background"]
        == PHI_LIGHT_THEME.markdown_code_block_background
    )
    assert variables["phi-markdown-link"] == PHI_LIGHT_THEME.markdown_link
    assert variables["phi-markdown-bullet"] == PHI_LIGHT_THEME.markdown_bullet
...
def test_light_theme_markdown_code_uses_aqua_without_background() -> None:
    ...
    console.print(
        render_chat_item(
            ChatItem(role="assistant", text="Use `phi` here."),
            theme=PHI_LIGHT_THEME,
        )
    )
...
def test_tool_chat_items_color_status_metadata_not_tool_name_or_results() -> None:
    ...
    red = "38;2;255;79;79"
    white = "38;2;201;209;217"

    assert green in success_output
    assert f"{white};48;2;13;17;23mread" in success_output
    assert f"{green};48;2;13;17;23mread" not in success_output
    assert f"{green};48;2;13;17;23m✓ read" not in success_output
    assert f"{green};48;2;13;17;23mcontents" not in success_output

    assert red in error_output
    assert f"{white};48;2;13;17;23m✗ bash" in error_output
    assert f"{red};48;2;13;17;23m✗ bash" not in error_output
    assert f"{red};48;2;13;17;23mfailed" not in error_output
...
async def test_streaming_transcript_deltas_do_not_force_scroll_end() -> None:
    app = PhiTuiApp(FakeSession(messages=[]))
...
def test_tui_app_uses_configured_theme_css_variables() -> None:
    app = PhiTuiApp(FakeSession(), tui_settings=TuiSettings(theme="high-contrast"))

    variables = app.get_theme_variable_defaults()

    assert variables["phi-screen-background"] == "#000000"
    assert variables["phi-prompt-background"] == "#1a1a1a"
    assert variables["phi-prompt-border"] == "#00ff66"


def test_tui_app_uses_light_theme_css_variables() -> None:
    app = PhiTuiApp(FakeSession(), tui_settings=TuiSettings(theme="phi-light"))

    variables = app.get_theme_variable_defaults()

    assert variables["phi-screen-background"] == "#ffffff"
    assert variables["phi-chrome-background"] == "#f3f4f6"
    assert variables["phi-muted-text"] == "#475569"
    assert variables["phi-prompt-background"] == "#f8fafc"
    assert variables["phi-prompt-border"] == "#2563eb"
...
def test_phi_dark_theme_uses_black_chat_backgrounds() -> None:
    theme = TuiSettings().resolved_theme

    assert theme.screen_background == "#0d1117"
    assert theme.transcript_background == "#0d1117"
    assert theme.prompt_background == "#161b22"
    assert theme.role_styles["user"].body.endswith("on #0d1117")
    assert theme.role_styles["assistant"].body.endswith("on #0d1117")


def test_phi_light_theme_uses_light_chat_backgrounds() -> None:
    theme = TuiSettings(theme="phi-light").resolved_theme
...
async def test_tui_app_shows_activity_indicator_while_running() -> None:
    ...
    async with app.run_test():
        ...
        assert prompt.styles.border.top[1].hex.lower() == "#e3b341"
        assert indicator.render().plain == "φ"
...
async def test_tui_app_theme_command_opens_picker_and_persists_selection(
    ...
    async with app.run_test() as pilot:
        ...
        assert app.tui_settings.theme == "phi-light"
        assert tui_settings_path().read_text(encoding="utf-8").find('"theme": "phi-light"') != -1
        assert app.get_theme_variable_defaults()["phi-screen-background"] == "#ffffff"
...
async def test_tui_app_new_command_starts_new_visible_state() -> None:
    app = PhiTuiApp(FakeSession(messages=[UserMessage(content="Earlier")]))
...
async def test_tui_app_tree_picker_branches_with_summary() -> None:
    ...
    async with app.run_test() as pilot:
        ...
        assert str(rendered_labels[0].spans[0].style) == _style_rgb(PHI_DARK_THEME.accent)
...
async def test_tui_login_saves_provider_key(
    ...
    assert (tmp_path / ".phi" / "credentials.json").read_text(encoding="utf-8")
...
async def test_tui_login_openai_codex_saves_oauth_credentials(
    ...
    assert tui_app.load_provider_settings().default_provider == "ollama"
    assert all("access-token" not in item.text for item in app.state.items)
    credentials = (tmp_path / ".phi" / "credentials.json").read_text(encoding="utf-8")
...
async def test_tui_login_preserves_existing_scoped_models_and_providers(
    ...
    assert FileCredentialStore(phi_home / "credentials.json").get("openrouter") == (
...
async def test_tui_logout_removes_stored_api_key(
    ...
    monkeypatch.setenv("HOME", str(tmp_path))
    credential_path = tmp_path / ".phi" / "credentials.json"
...
async def test_tui_logout_opens_stored_credential_provider_picker(
    ...
    monkeypatch.setenv("HOME", str(tmp_path))
    FileCredentialStore(tmp_path / ".phi" / "credentials.json").set("anthropic", "stored-key")
    app = PhiTuiApp(FakeSession())
...
async def test_tui_prompt_ctrl_c_clears_text() -> None:
    app = PhiTuiApp(FakeSession(messages=(UserMessage(content="User prompt"),)))
...
async def test_tui_prompt_worker_refreshes_directly() -> None:
    app = PhiTuiApp(FakeSession(events=[AgentStartEvent(), AgentEndEvent()]))
    refreshes = 0
...
async def test_tui_prompt_worker_shows_diagnostic_log_path_for_error_event(tmp_path: Path) -> None:
    class ErrorSession(FakeSession):
        def __init__(self) -> None:
            ...
            self.last_diagnostic_log_path = tmp_path / "phi-home" / "logs" / "agent-calls.jsonl"
...
async def test_tui_app_runs_initial_prompt() -> None:
    ...
    app = PhiTuiApp(session, initial_prompt="explain this repo")
...
async def test_run_tui_app_falls_back_to_first_credentialed_provider(
    ...
    monkeypatch.setattr(tui_app, "PhiTuiApp", FakeApp)
...
async def test_run_tui_app_opens_when_provider_login_is_missing(
    ...
    class FakeApp:
        def __init__(self, session: str, **kwargs: object) -> None:
            ...
            assert "/login ollama" in message
    ...
    assert calls == [f"create:{tmp_path}:qwen2.5:7b:ollama", "load:LoginRequiredProvider", "run"]
