...
from phi_coding.commands import create_default_command_registry
from phi_coding.prompt_templates import PromptTemplate
from phi_coding.skills import Skill
from phi_coding.tui.autocomplete import CompletionOption, build_completion_state
from phi_coding.tui.widgets import render_completion_suggestions
...
def test_theme_argument_completion_uses_theme_names() -> None:
    state = build_completion_state(
        ...
        skills=(),
        prompt_templates=(),
        theme_names=("phi-dark", "phi-light", "high-contrast"),
    )

    assert [item.display for item in state.items] == ["phi-dark", "phi-light"]
    assert state.selected is not None
    assert state.selected.apply("/theme phi-") == "/theme phi-dark"
