...
from phi_agent.messages import AgentMessage, AssistantMessage, ToolResultMessage
from phi_agent.tools import AgentTool, AgentToolResult, ToolCall
    ...
    *,
    ...
    while max_turns is None or turn <= max_turns:
        ...
        async for provider_event in provider.stream_response(
            ...
            elif isinstance(provider_event, ProviderToolSupportEvent):
                if not provider_event.supported:
                    yield ErrorEvent(
                        message=(
                            "This model does not support tool calling. "
                            "Coding tools are disabled for this turn."
                        ),
                        recoverable=True,
                    )
