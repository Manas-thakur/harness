"""Project instruction discovery for Phi coding sessions."""

from pathlib import Path

from phi_coding.resources import PhiResourcePaths, ResourceDiagnostic
...
def discover_project_context(
    paths: PhiResourcePaths | None = None,
...
def discover_project_context_with_diagnostics(
    ...
    resource_paths = paths or PhiResourcePaths()
...
def _context_file_candidates(paths: PhiResourcePaths) -> tuple[Path, ...]:
    ...
    if paths.cwd is not None:
        ...
        phi_paths = paths._paths()
        candidates.extend(
            [
                phi_paths.project_phi_dir(cwd) / "AGENTS.md",
                phi_paths.project_agents_dir(cwd) / "AGENTS.md",
            ]
        )
