"""Command-line entry point for Phi."""
...
from phi_agent.session import JsonlSessionStorage, SessionEntry, SessionStorage
from phi_ai import (
...
from phi_coding import __version__
from phi_coding.credentials import FileCredentialStore
...
from phi_coding.rendering import PrintOutputMode, create_event_renderer
...
from phi_coding.session_export import (
...
from phi_coding.tui import run_tui_app

app = typer.Typer(
    name="phi",
    help="Phi coding-agent harness.",
...
def setup_command(
    *,
    ...
    if provider.api_key_env not in environ:
        typer.echo(f"Set {provider.api_key_env} before running Phi with this provider.", err=True)
...
def main(
    ...
    setup_base_url: Annotated[
        str,
        typer.Option("--base-url", help="OpenAI-compatible base URL for `phi setup`."),
    ] = DEFAULT_OPENAI_COMPATIBLE_BASE_URL,
    setup_api_key_env: Annotated[
        str,
        typer.Option("--api-key-env", help="API key environment variable for `phi setup`."),
    ] = "OPENAI_API_KEY",
    setup_timeout_seconds: Annotated[
        float,
        typer.Option(
            "--timeout-seconds",
            help="HTTP timeout in seconds for `phi setup` provider requests.",
        ),
    ] = DEFAULT_OPENAI_COMPATIBLE_TIMEOUT_SECONDS,
    setup_max_retries: Annotated[
        int,
        typer.Option("--max-retries", help="Provider retry count for `phi setup`."),
    ] = DEFAULT_OPENAI_COMPATIBLE_MAX_RETRIES,
    setup_max_retry_delay_seconds: Annotated[
        float,
        typer.Option(
            "--max-retry-delay-seconds",
            help="Provider retry delay in seconds for `phi setup`.",
        ),
    ] = DEFAULT_OPENAI_COMPATIBLE_MAX_RETRY_DELAY_SECONDS,
    ...
    version: Annotated[
        bool,
        typer.Option("--version", help="Show Phi's version and exit."),
    ] = False,
) -> None:
    """Run the Phi CLI."""
    if version:
        typer.echo(f"phi {__version__}")
        raise typer.Exit()
...
def _parse_export_cli_args(args: list[str]) -> tuple[str, Path | None, str | None]:
    if not args:
        raise RuntimeError("Usage: phi export <session-id-or-jsonl> [--format html|jsonl] [output]")
    session_ref = args[0]
    output_path: Path | None = None
    export_format: str | None = None
    index = 1
    while index < len(args):
        ...
        if arg == "--format":
            ...
            if index >= len(args):
                raise RuntimeError(
                    "Usage: phi export <session-id-or-jsonl> [--format html|jsonl] [output]"
                )
...
def _resolve_export_source(
    ...
    if candidate_path.exists():
        ...
        return candidate_path, f"Phi session {candidate_path.stem}"
    ...
    title = record.title or f"Phi session {record.id}"
...
async def run_openai_print_mode(
    ...
    _warn_if_model_lacks_tool_support(selection.provider, selection.model)
...
async def run_print_mode(
    *,
    ...
    output: PrintOutputMode = PrintOutputMode.text,
    resource_paths: PhiResourcePaths | None = None,
...
def _warn_if_model_lacks_tool_support(provider: ProviderConfig, model: str) -> None:
    """Warn when an Ollama model cannot use Phi's coding tools."""
    ...
    if supports is False:
        typer.echo(
            f"Warning: model '{model}' does not support tool calling. "
            "Phi's coding tools (read/write/edit/bash) are disabled for this run. "
            "Pick a tool-capable model with `phi --model <name>` or `/model` in the TUI.",
            err=True,
        )
