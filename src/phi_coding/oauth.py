...
from phi_coding.credentials import OAuthCredential
...
async def _start_local_oauth_server(state: str) -> _LocalOAuthServer | None:
    host = environ.get("PHI_OAUTH_CALLBACK_HOST", "127.0.0.1")
...
def _oauth_html(message: str) -> str:
    ...
    return f'<!doctype html><meta charset="utf-8"><title>Phi OAuth</title><p>{escaped}</p>'
