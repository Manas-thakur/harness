"""Durable provider configuration for Phi coding sessions."""
...
from phi_ai.env import DEFAULT_OPENAI_COMPATIBLE_BASE_URL
from phi_coding.credentials import FileCredentialStore, credentials_path
from phi_coding.ollama import discover_ollama_models_with_capabilities
from phi_coding.paths import PhiPaths
from phi_coding.provider_catalog import BUILTIN_PROVIDER_CATALOG, ProviderKind
from phi_coding.thinking import (
...
class ProviderConfigError(ValueError):
    """Raised when Phi provider configuration is invalid."""
...
class ProviderSettings:
    """Phi provider settings loaded from Phi home."""
...
class ProviderSelection:
    """Resolved provider/model selection for a Phi run."""
...
def builtin_provider_configs() -> tuple[ProviderConfig, ...]:
    """Return Phi's built-in provider configs."""
    return tuple(
...
def provider_config_from_catalog_entry(name: str) -> ProviderConfig:
    ...
    for entry in BUILTIN_PROVIDER_CATALOG:
        ...
        return OpenAICompatibleProviderConfig(
            ...
            base_url=entry.base_url,
            api_key_env=entry.api_key_env,
            credential_name=entry.credential_name or None,
            models=entry.models,
            default_model=entry.default_model,
            context_windows=context_windows,
            thinking_levels=entry.thinking_levels,
...
def default_openai_provider_config() -> OpenAICompatibleProviderConfig:
    """Return Phi's default OpenAI-compatible provider entry."""
    provider = provider_config_from_catalog_entry("openai")
...
def provider_settings_path(paths: PhiPaths | None = None) -> Path:
    """Return the durable provider settings path."""
    return (paths or PhiPaths()).home / "providers.json"


def load_provider_settings(paths: PhiPaths | None = None) -> ProviderSettings:
    """Load durable provider settings, falling back to env-compatible defaults."""
    path = provider_settings_path(paths)
    if not path.exists():
        return _with_builtin_catalog_models(ProviderSettings(), paths=paths)
...
def save_provider_settings(settings: ProviderSettings, paths: PhiPaths | None = None) -> Path:
...
def _with_builtin_catalog_models(
    ...
    *,
    ...
    builtin_configs = {
        name: _apply_dynamic_models(provider) for name, provider in builtin_configs.items()
    }
    providers = tuple(
...
def _apply_dynamic_models(provider: ProviderConfig) -> ProviderConfig:
    """Return a provider config with dynamically-discovered models applied.

    For the Ollama provider, only models that declare the ``tools`` capability
    are included by default, since Phi's coding tools require tool-calling
    support. If no installed model supports tools, all discovered models are
    returned so the user can still pick one manually (Phi will warn at runtime
    when tools are unavailable).
    """
    if not (isinstance(provider, OpenAICompatibleProviderConfig) and provider.discover_models):
        return provider
    discovered = discover_ollama_models_with_capabilities(provider.base_url)
    if not discovered:
        return provider
    tool_models = [m.name for m in discovered if m.supports_tools]
    names = tool_models or [m.name for m in discovered]
    return replace(
        provider,
        models=tuple(names),
...
def _append_credentialed_builtin_providers(
    ...
    *,
    ...
    """Append built-in providers that already have stored Phi credentials."""
...
def _merge_provider_config(existing: ProviderConfig, incoming: ProviderConfig) -> ProviderConfig:
    ...
    if type(existing) is not type(incoming):
        return incoming
    if isinstance(incoming, OpenAICompatibleProviderConfig) and incoming.discover_models:
    ...
    *,
    ...
    if provider.thinking_levels is None:
        if isinstance(provider, OpenAICodexProviderConfig):
            return (
                "OpenAI Codex subscription can stream reasoning output, but Phi does "
...
def openai_compatible_config_from_provider(
    ...
    *,
    ...
    if provider.name == "openai" and provider.api_key_env == "OPENAI_API_KEY":
        base_url = environ.get("OPENAI_BASE_URL", provider.base_url)
    elif provider.name == "ollama":
        base_url = environ.get("OLLAMA_BASE_URL", provider.base_url)
...
def provider_has_usable_credentials(
    ...
    *,
    ...
    """Return whether Phi can attempt calls for this provider without prompting setup."""
    if isinstance(provider, OpenAICompatibleProviderConfig) and not provider.requires_api_key:
        return True
...
def _provider_from_json(data: object) -> ProviderConfig:
    ...
    default_model = _optional_string(data.get("default_model"), f"providers[{name}].default_model")
    ...
    requires_api_key = _bool_field(
        data.get("requires_api_key", True), f"providers[{name}].requires_api_key"
    )
    discover_models = _bool_field(
        data.get("discover_models", False), f"providers[{name}].discover_models"
    )
    if provider_type != "openai-compatible":
        if not models:
            raise ProviderConfigError(
                f"Provider field must be a non-empty string list: providers[{name}].models"
            )
        if not default_model:
            raise ProviderConfigError(
                f"Provider field must be a non-empty string: providers[{name}].default_model"
            )
