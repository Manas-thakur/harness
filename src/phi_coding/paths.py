"""Canonical filesystem paths for Phi user and project data."""
...
class PhiPaths:
    """Resolved Phi filesystem locations.

    Phi keeps durable application data under the user's home directory while also
    loading project-local resources from the active working directory.
    """

    home: Path = field(default_factory=lambda: Path.home() / ".phi")
    ...
    def logs_dir(self) -> Path:
        """Return Phi's user-level diagnostic log directory."""
    ...
    def user_skills_dir(self) -> Path:
        """Return Phi's user-level skills directory."""
        return self.home / "skills"

    @property
    def user_prompts_dir(self) -> Path:
        """Return Phi's user-level prompt templates directory."""
    ...
    def project_phi_dir(self, cwd: Path) -> Path:
        """Return the project-local Phi resource directory."""
        return cwd / ".phi"
    ...
    def project_skills_dir(self, cwd: Path) -> Path:
        """Return the project-local Phi skills directory."""
        return self.project_phi_dir(cwd) / "skills"

    def project_prompts_dir(self, cwd: Path) -> Path:
        """Return the project-local Phi prompt templates directory."""
        return self.project_phi_dir(cwd) / "prompts"
