"""Local credential storage for Phi provider credentials."""
...
class CredentialStoreError(ValueError):
    """Raised when Phi credential storage cannot be read or written."""


@dataclass(frozen=True, slots=True)
class OAuthCredential:
    """Refreshable OAuth credential persisted under Phi home."""
...
class ApiKeyCredential:
    """API-key credential persisted under Phi home."""
...
class FileCredentialStore:
    """Small JSON-backed provider credential store under Phi home."""
    ...
    def _load(self) -> dict[str, StoredCredential]:
        ...
        if not isinstance(raw, dict):
            raise CredentialStoreError("Phi credentials must be a JSON object")
        credentials: dict[str, StoredCredential] = {}
        for key, value in raw.items():
            if not isinstance(key, str):
                raise CredentialStoreError("Phi credential names must be strings")
...
def credentials_path(paths: PhiPaths | None = None) -> Path:
    """Return Phi's local provider credential path."""
    return (paths or PhiPaths()).home / "credentials.json"
...
def _credential_from_json(value: object) -> StoredCredential:
    ...
    if not isinstance(value, dict):
        raise CredentialStoreError("Phi credential values must be strings or objects")

    credential_type = value.get("type")
    if credential_type not in {"api_key", "oauth"}:
        raise CredentialStoreError("Phi credential object type must be api_key or oauth")
    ...
    if not isinstance(expires, int) or isinstance(expires, bool) or expires <= 0:
        raise CredentialStoreError("Phi oauth credential expires must be a positive integer")
...
def _string_field(
    ...
    if not isinstance(field, str) or not field.strip():
        raise CredentialStoreError(
            f"Phi {credential_type} credential field must be a non-empty string: {field_name}"
        )
    return field.strip()
