...
from phi_agent.messages import AgentMessage, AssistantMessage, UserMessage
...
from phi_agent.session.entries import SessionEntry
from phi_agent.session.tree import SessionTreeError, path_to_entry
...
from phi_coding.diagnostics import (
...
from phi_coding.skills import Skill, expand_skill_command, load_skills_with_diagnostics
from phi_coding.system_prompt import (
...
class SessionResources:
    """Phi-owned resources loaded around a coding session."""
...
class CodingSession:
    """Phi's coding-agent environment wrapper.
    ...
    def available_providers(self) -> tuple[str, ...]:
        """Return provider names Phi can call with available credentials."""
    ...
    def available_model_choices(self) -> tuple[ModelChoice, ...]:
        """Return provider/model choices Phi can call with available credentials."""
        if self._provider_settings is None:
    ...
    def context_window_tokens(self) -> int:
        """Return the active model's configured context window, or Phi's fallback."""
        provider = self._active_provider_config()
        if provider is None:
...
def _resolve_export_destination(
    ...
    *,
    ...
    if destination is None:
        ...
        return cwd / f"phi-session.{format}"

    resolved = destination if destination.is_absolute() else cwd / destination
    if resolved.suffix:
        return resolved
    name = session_path.stem if session_path is not None else "phi-session"
    return default_session_export_artifact_path(
        Path(name),
...
def _session_export_title(session: CodingSession) -> str:
    ...
    if manager is not None and session_id is not None:
        ...
        if record is not None and record.title:
            return record.title
    return f"Phi session {session_id}" if session_id is not None else "Phi Session Export"
...
def _load_session_resources(
    resource_paths: PhiResourcePaths,
...
def default_session_path(cwd: Path) -> Path:
    """Return Phi's default user-home session path for a project cwd."""
    return PhiPaths().default_session_path(cwd)
