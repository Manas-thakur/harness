"""Minimal Textual app for Phi coding sessions."""
...
from phi_ai import ProviderErrorEvent, ProviderEvent
from phi_ai.provider import CancellationToken
from phi_coding.commands import CommandRegistry, create_default_command_registry
from phi_coding.credentials import FileCredentialStore, OAuthCredential
from phi_coding.oauth import OAuthAuthInfo, OAuthPrompt, login_openai_codex
from phi_coding.ollama import model_supports_tools
from phi_coding.provider_catalog import (
...
from phi_coding.provider_runtime import create_model_provider
from phi_coding.session import (
    CodingSession,
    CodingSessionConfig,
    ModelChoice,
    SessionTreeChoice,
...
from phi_coding.thinking import DEFAULT_THINKING_LEVEL
from phi_coding.tui.adapter import TuiEventAdapter
from phi_coding.tui.autocomplete import (
    CompletionItem,
    CompletionOption,
    CompletionState,
...
from phi_coding.tui.state import TuiState, format_terminal_command_result_block
...
class PhiTuiApp(App[None]):
    ...
    Screen {
        layout: vertical;
        background: $phi-screen-background;
        color: $phi-screen-text;
    }

    Header {
        background: $phi-chrome-background;
        color: $phi-muted-text;
        dock: top;
    }

    Footer {
        background: $phi-chrome-background;
        color: $phi-chrome-text;
    }
    ...
    #sidebar {
        ...
        height: 1fr;
        padding: 1 1 0 1;
        background: $phi-sidebar-background;
        border-right: tall $phi-border;
    }

    PhiTuiApp.-hide-sidebar #sidebar {
        display: none;
    }

    PhiTuiApp.-hide-sidebar #main-pane {
        padding-left: 1;
    }
    ...
    #transcript {
        height: 1fr;
        border: none;
        background: $phi-transcript-background;
        padding: 0 0 0 2;
    ...
    #prompt {
        width: 1fr;
        height: auto;
        background: $phi-prompt-background;
        color: $phi-prompt-text;
        border: tall $phi-border;
        margin: 0;
        padding: 0 1;
        max-height: 8;
    }
    ...
    #prompt:focus {
        border: tall $phi-prompt-border;
    }

    #prompt.-shell-mode {
        border: tall $phi-accent;
    }
    ...
    #autocomplete {
        ...
        margin: 0 1 1 1;
        padding: 0 1;
        background: $phi-autocomplete-background;
        color: $phi-screen-text;
        border: tall $phi-prompt-border;
        overflow-y: auto;
    }
    ...
    ListView > ListItem.--highlight {
        background: $phi-highlight-background;
        color: $phi-highlight-text;
    }
    ...
    def get_theme_variable_defaults(self) -> dict[str, str]:
        """Return Phi-specific CSS variables for the selected TUI theme."""
        ...
        with Horizontal(id="workspace"):
            ...
            with Vertical(id="main-pane"):
                ...
                with Horizontal(id="prompt-row"):
                    yield Static("φ", id="prompt-prefix")
                    yield PromptInput(
                        placeholder="Ask Phi…  Enter to send, Shift+Enter for a newline",
                        id="prompt",
...
def _render_activity_indicator(theme: TuiTheme, *, frame: int, running: bool) -> Text:
    """Render the prompt prefix, turning Phi into a moving square while running."""
    if not running:
        return Text("φ", style=f"bold {theme.accent}")
...
def _api_key_login_providers(
    ...
    return tuple(
        ...
        if provider.kind != "openai-codex" and provider.requires_api_key
    )
...
def _theme_css_variables(theme: TuiTheme) -> dict[str, str]:
    return {
        "phi-screen-background": theme.screen_background,
        "phi-screen-text": theme.screen_text,
        "phi-chrome-background": theme.chrome_background,
        "phi-chrome-text": theme.chrome_text,
        "phi-muted-text": theme.muted_text,
        "phi-sidebar-background": theme.sidebar_background,
        "phi-border": theme.border,
        "phi-transcript-background": theme.transcript_background,
        "phi-prompt-background": theme.prompt_background,
        "phi-prompt-text": theme.prompt_text,
        "phi-prompt-border": theme.prompt_border,
        "phi-autocomplete-background": theme.autocomplete_background,
        "phi-accent": theme.accent,
        "phi-highlight-background": theme.highlight_background,
        "phi-highlight-text": theme.highlight_text,
        "phi-markdown-highlight": theme.markdown_heading,
        "phi-markdown-table-header": theme.markdown_table_header,
        "phi-markdown-table-border": theme.markdown_table_border,
        "phi-markdown-inline-code": theme.markdown_inline_code,
        "phi-markdown-code-block-background": theme.markdown_code_block_background,
        "phi-markdown-link": theme.markdown_link,
        "phi-markdown-bullet": theme.markdown_bullet,
...
def _tool_support_warning(provider: ProviderConfig, model: str) -> str | None:
    """Return a startup warning when an Ollama model cannot use coding tools."""
    if not (isinstance(provider, OpenAICompatibleProviderConfig) and provider.discover_models):
        return None
    supports = model_supports_tools(model, provider.base_url)
    if supports is False:
        return (
            f"Model '{model}' does not support tool calling. "
            "Coding tools are disabled. Use /model to pick a tool-capable model."
        )
    return None
...
async def run_tui_app(
    *,
    ...
    tool_warning = _tool_support_warning(selection.provider, selection.model)
    try:
        ...
        if tool_warning and startup_message is None:
            startup_message = tool_warning
