"""Small Textual widgets for Phi's interactive TUI."""
...
from phi_coding.tui.autocomplete import CompletionState
from phi_coding.tui.config import PHI_DARK_THEME, TuiRoleStyle, TuiTheme
from phi_coding.tui.state import ChatItem, TuiState

PHI_SIDEBAR_LOGO = "φ ≈ 1.618"
...
class PhiMarkdownBlock(MarkdownBlock):
    """Markdown block that applies Phi's themed inline link color."""
    ...
    def _token_to_content(self, token: Any) -> Any:
        ...
        if not isinstance(markdown, ThemedMarkdownWidget):
            return content
        link_style = TextualStyle.parse(markdown.phi_link_style)
        spans = []
...
class ThemedMarkdownWidget(TextualMarkdown):
    """Textual Markdown widget reserved for Phi transcript streaming."""

    BLOCKS = {**TextualMarkdown.BLOCKS, "paragraph_open": PhiMarkdownBlock}
    ...
    def __init__(
        ...
        *,
        ...
        self.phi_link_style = theme.markdown_link
...
class TranscriptView(VerticalScroll):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._render_theme: TuiTheme = PHI_DARK_THEME
...
def render_session_sidebar(
    ...
    *,
    ...
    equation = Text(PHI_SIDEBAR_LOGO, style=f"bold {theme.prompt_text}")
...
class ThemedCodeBlock(CodeBlock):
    """Rich Markdown code block with Phi's themed background color."""
...
class ThemedMarkdown(Markdown):
    """Markdown renderer with Phi's softer heading/accent colors."""
