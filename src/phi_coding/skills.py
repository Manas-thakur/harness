...
def load_skills(paths: PhiResourcePaths | None = None) -> list[Skill]:
    """Load markdown skills from Phi and `.agents` resource directories.
...
def parse_skill_invocation(text: str) -> SkillInvocation | None:
    """Parse Phi's expanded skill invocation message format."""
    match = re.match(
