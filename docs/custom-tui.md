...
Phi's Textual app is one frontend, not the agent architecture itself. A custom
...
The reusable `phi_agent` package must stay independent of terminal frameworks,
widgets, keybindings, local config paths, and slash-command UX.

## Recommended Boundary

Build custom frontends against `phi_coding.session.CodingSession`, not directly
...
Slash commands are owned by `phi_coding`, not by `phi_agent`.
...
output such as `/session`, `/resume`, `/hotkeys`, and `/tree`.
...
An empty string means `/compact` was run with no extra instructions and Phi
...
```python
from phi_coding.tui import CompletionOption
...
settings from `~/.phi/tui.json` through `phi_coding.tui.load_tui_settings()`,
...
The built-in themes are `phi-dark` and `high-contrast`. They resolve to
`TuiTheme` objects used by the Textual app and widgets; custom frontends can
reuse those objects or define their own palette outside `phi_agent`.

## Session Switching

Session records are managed by `phi_coding.session_manager.SessionManager`.
...
- `phi_agent` internals that are not part of the event, message, tool, harness, or session primitives

## Verification Checklist

A custom TUI should prove these behaviors before it is considered compatible:

- `phi_agent` has no dependency on the frontend framework.
...
- Keybindings and themes are owned by the frontend and do not leak into `phi_agent`.
