...
Phase 2 creates Phi's provider/model streaming layer in `phi_ai`.

The purpose of this layer is to let Phi talk to model APIs without letting provider-specific details leak into the reusable agent loop.
...
```text
src/phi_ai/events.py              provider-neutral stream events
src/phi_ai/provider.py            ModelProvider protocol
src/phi_ai/fake.py                deterministic fake provider for tests
src/phi_ai/env.py                 environment-based provider configuration
src/phi_ai/openai_compatible.py   OpenAI-compatible chat completions adapter
```
...
Phi does not want the agent loop to know those details.
...
The future agent loop can consume the same Phi event stream regardless of the backend model.
...
This is the seam that lets Phi support multiple backends later.

## Fake provider

`FakeProvider` replays scripted provider events.

This is important because Phi should be testable without:
