...
Phase 12 adds Phi's first interactive terminal UI using Textual.
...
Phi now has a minimal interactive TUI that:
...
- stores the early default TUI session at `.phi/sessions/default.jsonl`
...
```bash
phi --model gpt-4.1-mini --cwd /path/to/project
```
...
Phi now follows that same boundary with Python-native Textual:
...
`phi_agent` still has no dependency on Textual, Rich, Typer, or terminal UI behavior.
...
```text
src/phi_coding/tui/state.py
src/phi_coding/tui/adapter.py
```
...
def default_session_path(cwd: Path) -> Path:
    return cwd / ".phi" / "sessions" / "default.jsonl"
```
