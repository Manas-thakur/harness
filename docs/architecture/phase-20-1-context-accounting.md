...
Phase 20.1 tightens Phi's context accounting and makes TUI refreshes follow the
same event stream that updates the active agent transcript.

## What Was Added

`phi_coding.context_window` now exposes `ContextUsageEstimate`, a structured
snapshot with:
...
This keeps the reusable `phi_agent` package UI-free: it only emits portable
events. Textual-specific rendering remains in `phi_coding.tui`.
