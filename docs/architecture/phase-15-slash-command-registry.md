...
Phase 15 replaces Phi's hardcoded slash-command handling with a small registry in
`phi_coding`.
...
Phi now has a `CommandRegistry` that can:
...
## Why this belongs in `phi_coding`
...
The reusable `phi_agent` package remains independent of slash commands, Textual,
Typer, local config directories, and Phi-specific product behavior.
...
Phi mirrors the commands that map cleanly onto existing Phi capabilities:

- `/session` maps to Phi's existing session status/details output.
- `/hotkeys` reports Phi's current common TUI shortcuts.
- `/quit` is the only registered exit command; older Phi-specific `/exit` and
  `/q` aliases are intentionally not retained.
- `/model`, `/login`, `/new`, `/compact`, `/resume`, `/reload`, `/name`,
  `/theme`, and `/export` exist in Phi's command registry.

Phi intentionally removes older Phi-specific diagnostic commands from the
