...
Phase 20.2 makes thinking mode an explicit Phi coding-session setting and adds
a TUI control for changing it.

## What Was Added

`phi_coding.thinking` defines Phi's supported thinking modes:
...
The default is `medium`, matching Pi's default reasoning-depth preference. Phi
...
mode that Phi cannot safely send.
...
This keeps thinking state in `phi_coding`, while `phi_agent.session` remains the
...
Phi does not register a standalone `/thinking` command in the default command
...
all models for that provider. Phi's built-in providers declare thinking support
...
`thinking_models` only for models that actually support Phi's configured
thinking parameter.
...
Phi records an explicit reason when controls are unavailable:
...
This keeps the durable provider metadata in `phi_coding`, the provider-specific
runtime knob in `phi_ai`, and the display choice in CLI/TUI code.
...
and `xhigh`, with defaults and support varying by model. Phi's direct OpenAI
entry maps Phi's normalized levels to `reasoning_effort` only for configured
reasoning-capable models.

Codex clients expose `model_reasoning_effort` for supported models. Phi mirrors
...
adaptive/effort controls, with support changing by model family. Phi maps its
...
level, Phi coerces to that model/provider default.
...
`phi_agent` forwards those events without recording them as durable assistant
