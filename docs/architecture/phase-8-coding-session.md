...
Phase 8 adds Phi's first coding-session environment wrapper.
...
Phi now has a `CodingSession` that combines:
...
## Why this lives in `phi_coding`

`phi_agent` owns reusable primitives:
...
`phi_coding` owns the coding-agent environment around those primitives:
...
So `CodingSession` belongs in `phi_coding`.
...
from phi_coding import CodingSession, CodingSessionConfig
from phi_agent.session import JsonlSessionStorage
...
For a new empty session, Phi appends:
...
After the run completes, all new harness messages are appended as `MessageEntry` records. Phi also appends a `LeafEntry` pointing at the newest message entry.
...
Later phases can make persistence more incremental if Phi needs crash recovery during a streaming response.
