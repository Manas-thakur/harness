...
Phase 11 adds a small rendering boundary for Phi's non-interactive CLI modes.

The implementation lives in:

```text
src/phi_coding/rendering/
```

## What was added

Phi now has event renderers that consume `phi_agent` events outside the portable harness layer:

- `FinalTextRenderer` prints only the final assistant answer.
- `JsonEventRenderer` writes every agent event as JSON Lines.
- `TranscriptRenderer` preserves Phi's previous live transcript behavior.
- `PrintOutputMode` selects `text`, `json`, or `transcript` output.

The CLI exposes this with:

```bash
phi --output text "summarize this project"
phi --output json "summarize this project"
phi --output transcript "summarize this project"
```
...
Phi now follows the same architectural boundary:

```text
phi_agent     portable event-producing harness
phi_coding    CLI mode selection and event rendering
future TUI    another consumer of the same event stream
```

This keeps `phi_agent` free of Typer, Rich, Textual, terminal behavior, and UI policy.
...
Transcript mode preserves Phi's earlier print-mode behavior:
...
This is useful while Phi does not yet have a full interactive TUI.
...
The next phase can build the Textual TUI behind the same event boundary, or first add optional Rich styling inside `phi_coding` without changing `phi_agent`.
