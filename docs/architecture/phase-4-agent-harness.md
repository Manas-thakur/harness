...
Phi's architecture separates three layers:
...
Those responsibilities belong to later `phi_coding` phases.
...
If a behavior depends on terminal rendering, project files, user config directories, or slash commands, it belongs outside `phi_agent`.
