...
Phi retries transient provider failures in `phi_ai`, where HTTP status codes and
transport exceptions are visible. This keeps retry classification out of
`phi_agent` while still allowing the portable agent loop to surface progress.
...
`phi_agent` does not decide whether an HTTP response is retryable. It only
