...
This section explains Phi's architecture as it is built, one phase at a time.

Phi is intentionally developed in small layers. Each layer should answer three questions:
...
phi_coding   CLI app, resources, skills, extensions, commands, UI integration
```

The most important boundary is that `phi_agent` should stay portable. It can define the reusable agent brain, but it should not know about VS Code, Textual, Rich rendering, local config directories, slash commands, or project-specific prompts.
...
- [Phase 13: Phi Home, Paths, and `.agents` Resources](phase-13-paths-agents-resources.md)
