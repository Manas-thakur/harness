...
Phase 17 adds prompt autocomplete to Phi's Textual TUI while keeping the logic in
`phi_coding`.

The implementation lives in:

```text
src/phi_coding/tui/autocomplete.py
src/phi_coding/tui/app.py
src/phi_coding/tui/widgets.py
```

## What was added

Phi now builds completion suggestions for the prompt input from coding-session
metadata:
...
When the prompt starts with `/`, Phi shows matching slash commands.
...
When the prompt starts with `/skill:`, Phi shows matching loaded skills.
...
Prompt templates are already loaded and available on `CodingSession`, but Phi
...
Autocomplete stays out of `phi_agent`.

The reusable agent harness has no dependency on Textual, command registries,
skills, prompt templates, or Phi resource paths. Those remain application
concerns owned by `phi_coding`.
