...
Phase 3 adds Phi's first real agent engine: a pure loop that connects a model provider, a transcript, and a set of tools.

The loop lives in:

```text
src/phi_agent/loop.py
```

It is called “pure” because it does not know about the CLI, Rich, Textual, slash commands, local session files, or project-specific resources. It only works with provider-neutral Phi types.
...
Like Pi, Phi's loop does not impose a default turn limit. It continues until the assistant stops requesting tools or another normal stop condition occurs.
...
This keeps Phi's reusable agent package independent from coding-agent-specific behavior.
...
If behavior requires CLI flags, terminal rendering, local config paths, slash commands, or project resources, it belongs outside `phi_agent.loop`.
