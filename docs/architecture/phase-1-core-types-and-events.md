...
Phase 1 creates the shared vocabulary for Phi's future agent loop.

Before Phi can talk to a model, execute tools, save sessions, or render a UI, every layer needs to agree on the shape of the data flowing through the system. That is what the core message, tool, JSON, and event types provide.

## What was added

Phase 1 added four modules in `phi_agent`:

```text
src/phi_agent/types.py      JSON-like shared type aliases
src/phi_agent/messages.py   transcript message models
src/phi_agent/tools.py      tool calls, tools, and tool results
src/phi_agent/events.py     event models emitted by the agent layer
```
...
Tools are how the assistant asks Phi to interact with the environment.
...
Future provider adapters will translate provider-specific tool call payloads into this neutral Phi shape.

### `AgentTool`

An `AgentTool` describes a tool Phi can expose to a model.
...
Phi defines shared aliases for:
...
Phi currently defines events for:
...
Providers translate external model streams into Phi messages, tool calls, and
...
If a type belongs to the reusable agent brain, it lives in `phi_agent`.
