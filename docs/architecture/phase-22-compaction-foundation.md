...
This phase started Phi's compaction and context-management work. It now includes
...
```text
src/phi_agent/session/entries.py
src/phi_agent/session/memory.py
src/phi_coding/context_window.py
...
Phi needs compaction to reduce the active provider context while preserving the
...
Phi now has deterministic approximate context-size helpers in `phi_coding`:
...
without adding tokenizer policy to `phi_agent`.

Phi enables Pi-style automatic compaction by default using:
...
after a model response, Phi asks the active provider for a structured summary,
appends a `CompactionEntry`, and rebuilds the in-memory transcript. Phi also
attempts one compact-and-retry cycle after provider errors that look like
context overflow.

## Manual Compaction

Phi now supports model-generated manual compaction in the TUI:

```text
/compact [instructions]
```

The command uses Phi's built-in Pi-style compaction prompt. Optional
instructions are appended to that prompt as extra focus. The generated summary is
stored in a `CompactionEntry`, followed by a new leaf pointer. Phi then replays
...
This foundation is in `phi_agent` because replaying session entries is a
portable harness concern. It does not know about slash commands, Textual, Rich,
Phi home paths, token thresholds, or which model creates a summary.
