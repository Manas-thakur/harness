...
Phase 10 adds Phi's canonical system prompt builder.

The implementation lives in:

```text
src/phi_coding/system_prompt.py
```

## What was added

Phi can now build a deterministic Pi-style system prompt from:

- Phi's default coding-agent identity
- enabled tools
...
Earlier phases added tools, skills, prompt templates, and coding sessions. Before this phase, the CLI used a small local prompt builder that only listed tools. Phi now has one shared prompt assembly layer that can be used by print mode, coding sessions, and later UI/session frontends.

## Default prompt shape

The default prompt starts with Phi's identity:
...
If `custom_prompt` is supplied, it replaces the default Phi identity, tools, and guidelines sections.

Phi still appends these after a custom prompt, matching Pi's behavior:
...
<available_skills>
  <skill>
    ...
    <location>/home/user/.phi/skills/python-testing/SKILL.md</location>
  </skill>
</available_skills>
```

Phi includes the skills section only when the `read` tool is enabled. This mirrors Pi: the model should use `read` to load the full skill file when a task matches a skill description.
