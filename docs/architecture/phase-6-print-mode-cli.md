...
Phase 6 wired Phi's provider layer, agent harness, and built-in coding tools into
...
The `phi` command can run a single prompt in print mode with `-p`/`--prompt`:

```bash
phi -p "write tests for main.py"
phi --prompt "explain this repo"
phi --model gpt-5.5 -p "summarize README.md"
```

Without `-p`/`--prompt`, Phi starts the interactive TUI. A positional prompt is
treated as the initial TUI prompt:

```bash
phi
phi "explain this repo"
```

The original Phase 6 command:

1. loads OpenAI-compatible provider settings from the environment
2. creates Phi's built-in coding tools
...
Print mode uses Phi's configured provider settings.
...
```bash
phi --cwd /path/to/project -p "inspect the tests"
```
...
- full Phi system prompt assembly
- discovered project instructions
- loaded skills and prompt templates
- built-in coding tools
- append-only session persistence under `~/.phi/sessions/`
...
The CLI lives in `phi_coding`. It depends on provider settings,
`CodingSession`, and renderers, but the reusable `phi_agent` package still has
