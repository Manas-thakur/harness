...
`phi_agent.AgentHarness` now owns two prompt queues:
...
Queue ownership lives in `phi_agent`, prompt expansion and persistence stay in
`phi_coding`, and Textual only decides keybindings and presentation. Provider
