...
`~/.phi/tui.json`. This lets users remap the command palette, completion
navigation, session picker, cancellation, and quit keys while keeping the
configuration in `phi_coding.tui` instead of the reusable agent harness.

The same TUI settings file now supports named built-in themes. `phi-dark`
...
blocks stay visually consistent without adding UI policy to `phi_agent`.
...
The Textual footer now carries Phi's shortcut hints through ordinary visible
...
completions are open, and switches again while an agent turn is running. Phi
...
that token without making Textual a dependency of `phi_agent`. Because
...
The built-in theme set now includes `phi-light` alongside `phi-dark` and
`high-contrast`. Theme choice stays in `phi_coding.tui` configuration and feeds
...
```bash
git fetch origin
git worktree add /tmp/phi-tui-validate origin/main
cd /tmp/phi-tui-validate
uv run phi
```
...
5. Check the light theme by writing this file:
   ...
   to `~/.phi/tui.json`, restarting `uv run phi`, and confirming the app uses a
   light palette with readable transcript, sidebar, footer, and prompt colors.
   Restore your preferred theme after the check.

## Boundaries

These changes live in `phi_coding.tui`. The command registry still owns command
metadata, and `phi_agent` remains unaware of Textual, keybindings, slash
