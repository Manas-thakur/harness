...
Phi now has a `ResourceDiagnostic` type for non-fatal resource discovery notes.
It records:
...
If one directory contains two skills with the same name, Phi keeps the first
...
That keeps discovery diagnostics in `phi_coding`, where local resources,
commands, and UI behavior live. The reusable `phi_agent` package remains
