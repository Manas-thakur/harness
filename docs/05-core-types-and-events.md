...
Phase 1 defines the provider-neutral objects that later Phi phases share.

## Why these types exist

The provider layer, agent loop, harness, tools, sessions, and UI should not exchange provider-specific objects.
Instead, they use Phi's own message, tool, result, and event models.

## Messages

Messages live in `phi_agent.messages`:
...
Events live in `phi_agent.events`. They describe progress from the portable agent layer:
...
payloads into Phi types before the agent loop or frontends see them.
